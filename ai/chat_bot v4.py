...
llm = ChatGoogleGenerativeAI(model=google_model, temperature=0.2)
...
def agent_node1(state: State): # ... (code from original, no changes needed)
...
def summarize_conversation1(state: State): # ... (no changes needed)
    # ... (code from original)
...
def summarize_conversationBw4Commit(state: State):
...
def summarize_conversation(state: State) -> Dict[str, Any]:
    ...
    # 2. Create a prompt that instructs the LLM to generate a structured summary
    summarize_prompt = f"""
    Please provide a structured summary of the following conversation.
    Analyze the entire dialogue to determine the overall sentiment, identify any issues that were not resolved,
    and list all the sources that were mentioned or used to formulate the answers.
    ...
    Provide the output in a JSON format matching this exact schema:
    {{
        "summary": "A concise summary of the entire conversation.",
        "sentiment": "An integer representing the overall sentiment of the conversation, from -2 (very negative) to +2 (very positive).",
        "unresolved_tickets": ["A list of service channels (e.g., 'ATM', 'POS') with issues that were not fully resolved."],
        "all_sources": ["A list of all unique sources referenced in the conversation (e.g., 'PDF Content', 'Web Search')."]
    }}
    """
    
    # 3. Invoke the LLM with the structured output schema
    structured_llm = llm.with_structured_output(Summary)
    summary_obj = structured_llm.invoke(summarize_prompt)

    # 4. Compile the final 'metadatas' dictionary for logging
    final_answer = state.get("final_answer")
    
    # Safely get the user's last question (second to last message)
    last_user_question = ""
    if len(messages) > 1:
    ...
    metadata_dict = {
        # Data from the final answer of the current turn
        ...
        # Data from the new summary of the entire conversation
    ...
    # 5. Return the dictionary to update the state
    return {
